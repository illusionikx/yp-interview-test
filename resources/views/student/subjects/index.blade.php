<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
            <x-back-button :href="route('student.home')">{{ __('Back to home') }}</x-back-button>
            ...
            <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 overflow-hidden shadow-sm rounded-lg p-6">
                <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">{{ __('1. Choose a subject') }}</h3>
                <p class="text-sm text-gray-600 dark:text-gray-400 mb-4">{{ __('You may enroll in as many subjects as you like — there is no limit on how many classes you can hold at once.') }}</p>

                @if ($subjects->isEmpty())
                    <div class="text-center">
                        <h3 class="text-xl font-semibold text-gray-900 dark:text-white">{{ __('No subjects available') }}</h3>
                        <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">{{ __('There are no subjects open for enrollment right now.') }}</p>
                    </div>
                @else
                    <form method="GET" action="{{ route('student.subjects.index') }}" class="flex flex-wrap items-end gap-3">
                        <div>
                            ...
                            <select
                                ...
                                class="mt-1 block w-full sm:w-72 rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                            >
                                <option value="">{{ __('-- Select a subject --') }}</option>
                                @foreach ($subjects as $subject)
                                    <option value="{{ $subject->id }}" @selected($selectedSubject && $selectedSubject->id === $subject->id)>
                                        {{ $subject->name }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        <x-primary-button type="submit">{{ __('Choose') }}</x-primary-button>
                    </form>
                @endif
            </div>

            {{-- Step 2/3: select class + enroll --}}
            @if ($selectedSubject)
                <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 overflow-hidden shadow-sm rounded-lg p-6">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">
                        {{ __('2. Select a class for :subject', ['subject' => $selectedSubject->name]) }}
                    </h3>
                    ...
                    @forelse ($sections as $section)
                        @if ($loop->first)
                            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                <thead>
                                    <tr>
                                        <th class="px-4 py-2 text-left text-sm font-semibold text-gray-500 dark:text-gray-400">{{ __('Class') }}</th>
                        ...
                        @endif
                                    @php
                                        ...
                                        // ENR-11: enroll is offered ONLY when the window is currently
                                        // open (not 'opens', not 'closed') — the same $canApply gate
                                        // show.blade.php already applies; this is UX only, the real
                                        // enforcement boundary is EnrollmentController@store's
                                        // server-side windowStatus() re-check.
                                                ...
                                                <div class="space-y-1">
                                                    ...
                                                    @if ($canApply)
                                                        <form method="POST" action="{{ route('student.sections.enroll', $section) }}">
                                                            @csrf
                                                            <x-primary-button type="submit">{{ __('Enroll') }}</x-primary-button>
                                                        </form>
                                                    @endif
                                                </div>
                    ...
                    @empty
                        <div class="text-center">
                            <h3 class="text-xl font-semibold text-gray-900 dark:text-white">{{ __('No classes yet') }}</h3>
                            <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">{{ __('This subject has no classes open for enrollment right now.') }}</p>
                        </div>
                    @endforelse
                </div>
                ...
                @foreach ($sections as $section)
                    ...
                    @if ($ownEnrollment && $ownEnrollment->status === \App\Enums\EnrollmentStatus::Enrolled)
                        <x-modal name="withdraw-section-{{ $section->id }}" focusable>
                            <div class="p-6">
                                <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100">{{ __('Withdraw from Class') }}</h2>
